...
The Orchestrator forwards both spawn requests to TL via Send a message to the team-lead. TL fulfills them per the generic spawn protocol. The Orchestrator updates state: add both to `active_agents`.
...
3. If the Plan Evaluator and Architect are stuck (3+ REVISE cycles), the Plan Evaluator will escalate to the Orchestrator. The Orchestrator forwards the unresolved issue to TL via the platform messaging mechanism for user presentation.
...
2. Send shutdown requests to TL for the Gate Author pair and Plan Evaluator: Send a message to the team-lead: "Shutdown request: name=generator. Reason: Phase 2 complete." (and similarly for evaluator and plan-evaluator).
